...
use libcrux_secrets::{Classify, ClassifyRef, ClassifyRefMut, Declassify};
...
fn ccm_two_byte_aad_len_encoding() {
    ...
    let k: Ccm128Key = [0; AesCcm128::KEY_LEN].classify().into();
    let nonce: Ccm128Nonce = [0; AesCcm128::NONCE_LEN].classify().into();
    let mut tag: Ccm128Tag = [0; AesCcm128::TAG_LEN].classify().into();

    // unsafely create a slice that is too long
    let aad = vec![8; 512];

    let pt = [42u8; 1].classify();
    let mut ct = [0u8; 1];
    k.encrypt(&mut ct, &mut tag, &nonce, &aad, &pt).unwrap();

    let mut pt_decrypted = [0u8; 1].classify();
    k.decrypt(&mut pt_decrypted, &nonce, &aad, &ct, &tag)
        .unwrap();

    assert_eq!(pt.declassify(), pt_decrypted.declassify());
}
...
fn ccm_six_byte_aad_len_encoding_multi_block_plaintext() {
    ...
    let k: Ccm128Key = [0xddu8; AesCcm128::KEY_LEN].classify().into();
    let nonce: Ccm128Nonce = [0x11u8; AesCcm128::NONCE_LEN].classify().into();
    let mut tag: Ccm128Tag = [0; AesCcm128::TAG_LEN].classify().into();
    ...
    k.encrypt(&mut ct, &mut tag, &nonce, &aad, &pt.classify_ref())
        .unwrap();

    let mut pt_decrypted = vec![0u8; 64];
    k.decrypt(pt_decrypted.classify_ref_mut(), &nonce, &aad, &ct, &tag)
        .unwrap();
...
fn ccm_six_byte_aad_len_encoding_rejects_tampered_ciphertext() {
    ...
    let pt = [42u8; 16].classify();
    let mut ct = [0u8; 16];
    k.encrypt(&mut ct, &mut tag, &nonce, &aad, &pt).unwrap();

    ct[0] ^= 1; // flip one bit

    let mut pt_decrypted = [0u8; 16].classify();
    let err = k
...
fn ccm_six_byte_aad_len_encoding_aad_affects_tag() {
    ...
    let k: Ccm128Key = [0; AesCcm128::KEY_LEN].classify().into();
    let nonce: Ccm128Nonce = [0; AesCcm128::NONCE_LEN].classify().into();
    let mut tag1: Ccm128Tag = [0; AesCcm128::TAG_LEN].classify().into();
    let mut tag2: Ccm128Tag = [0; AesCcm128::TAG_LEN].classify().into();
    ...
    assert_eq!(ct1, ct2);
    assert_ne!(tag1.as_ref().declassify(), tag2.as_ref().declassify());
}
...
fn ccm_nist_kat() {
    ...
    let k: [u8; AesCcm128ShortTag::KEY_LEN] = hex::decode("404142434445464748494a4b4c4d4e4f")
        ...
        .try_into()
        .unwrap();
    let k: Key = k.classify().into();
    let nonce: [u8; AesCcm128ShortTag::NONCE_LEN] = hex::decode("101112131415161718191a1b")
        .unwrap()
        .try_into()
        .unwrap();
    let nonce: Nonce = nonce.classify().into();

    let mut tag: Tag = [0; AesCcm128ShortTag::TAG_LEN].classify().into();
