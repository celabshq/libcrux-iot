...
theorem accumulating_ntt_multiply_binomials_fc
  ...
  -- INC-2b.A: lhs bound is 4095 (not 3328). The only fact that is sensitive to that is
  -- the even-lane 2^25 delta bound, and it is recovered by using the TIGHT conjunct of
  -- `montgomery_reduce_element_spec` (`h_tight_imp` below) instead of the weak
  -- `|bj_zeta| ≤ 3328 + 1665` post — exactly as the `_fill_cache` sibling does. With
  -- `|bj_zeta| ≤ 3328` both deltas are ≤ 4095*3328 + 4095*3328 = 27,256,320 ≤ 2^25.
  -- The POST constant 2^25 is unchanged; `h_b` (rhs) stays at 3328 because only the rhs
  -- feeds the Montgomery reduce.
  -- ===== Setup =====
  ...
  have h_ai : ai_v.val.natAbs ≤ 4095 := h_a ⟨2 * i.val, h_2i_lt⟩
  have h_bi : bi_v.val.natAbs ≤ 3328 := h_b ⟨2 * i.val, h_2i_lt⟩
  have h_aj : aj_v.val.natAbs ≤ 4095 := h_a ⟨2 * i.val + 1, h_2i1_lt⟩
  ...
  have h_ai_bi_val : ai_bi.val = ai_v.val * bi_v.val := by
    have h_bnd : (ai32.val * bi32.val).natAbs < 2^31 := by
      ...
      have : ai_v.val.natAbs * bi_v.val.natAbs ≤ 4095 * 3328 := by
        exact Nat.mul_le_mul h_ai h_bi
      rw [h]
      have : (4095 * 3328 : Nat) < 2^31 := by decide
      omega
  ...
  -- Also recover the legacy modq form: bj_zeta * 2^16 ≡ bj_zeta_ (mod q), AND the TIGHT
  -- canonical bound `|bj_zeta| ≤ 3328` (the conditional conjunct of the legacy spec).
  ...
  -- This is what makes the even-lane 2^25 delta bound hold at lhs ≤ 4095.
  ...
  have h_aj_bj_zeta_val : aj_bj_zeta.val = aj_v.val * bj_zeta.val := by
    have h_bnd : (aj32.val * bj_zeta32.val).natAbs < 2^31 := by
      ...
      have h_mul : aj_v.val.natAbs * bj_zeta.val.natAbs ≤ 4095 * 3328 :=
        Nat.mul_le_mul h_aj h_bj_zeta_canon
      have : (4095 * 3328 : Nat) < 2^31 := by decide
      omega
  ...
  -- Even-delta bound: |ai*bi + aj*bj_zeta| ≤ 4095·3328 + 4095·3328 = 27,256,320 ≤ 2^25.
  have h_sum_e_bnd : (ai_bi.val + aj_bj_zeta.val).natAbs ≤ 4095 * 3328 + 4095 * 3328 := by
    rw [h_ai_bi_val, h_aj_bj_zeta_val]
    have h_e1 : (ai_v.val * bi_v.val).natAbs ≤ 4095 * 3328 := by
      rw [Int.natAbs_mul]; exact Nat.mul_le_mul h_ai h_bi
    have h_e2 : (aj_v.val * bj_zeta.val).natAbs ≤ 4095 * 3328 := by
      rw [Int.natAbs_mul]; exact Nat.mul_le_mul h_aj h_bj_zeta_canon
  ...
  have h_sum_e_val : ai_bi_aj_bj.val = ai_bi.val + aj_bj_zeta.val := by
    have h_bnd : (ai_bi.val + aj_bj_zeta.val).natAbs < 2^31 := by
      have h_le : (4095 * 3328 + 4095 * 3328 : Nat) < 2^31 := by decide
      omega
  ...
  have h_delta_e_bnd : ai_bi_aj_bj.val.natAbs ≤ 2^25 := by
    rw [h_sum_e_val]
    have : (4095 * 3328 + 4095 * 3328 : Nat) ≤ 2^25 := by decide
    omega
  ...
  have h_ai_bj_val : ai_bj_p.val = ai_v.val * bj_v.val := by
    have h_bnd : (ai32.val * bj32.val).natAbs < 2^31 := by
      ...
      have h_mul : ai_v.val.natAbs * bj_v.val.natAbs ≤ 4095 * 3328 :=
  ...
  have h_aj_bi_val : aj_bi_p.val = aj_v.val * bi_v.val := by
    have h_bnd : (aj32.val * bi32.val).natAbs < 2^31 := by
      ...
      have h_mul : aj_v.val.natAbs * bi_v.val.natAbs ≤ 4095 * 3328 :=
  ...
  have h_sum_o_bnd : (ai_bj_p.val + aj_bi_p.val).natAbs ≤ 2 * 4095 * 3328 := by
    rw [h_ai_bj_val, h_aj_bi_val]
    have h_e1 : (ai_v.val * bj_v.val).natAbs ≤ 4095 * 3328 := by
      rw [Int.natAbs_mul]; exact Nat.mul_le_mul h_ai h_bj
    have h_e2 : (aj_v.val * bi_v.val).natAbs ≤ 4095 * 3328 := by
...
/--
info: 'libcrux_iot_ml_kem.Polynomial.NttMultiply.accumulating_ntt_multiply_binomials_fc' depends on axioms: [propext,
 Classical.choice,
 Quot.sound]
-/
#guard_msgs in
#print axioms accumulating_ntt_multiply_binomials_fc

/--
info: 'libcrux_iot_ml_kem.Polynomial.NttMultiply.accumulating_ntt_multiply_fc' depends on axioms: [propext,
 Classical.choice,
 Quot.sound]
-/
#guard_msgs in
#print axioms accumulating_ntt_multiply_fc
