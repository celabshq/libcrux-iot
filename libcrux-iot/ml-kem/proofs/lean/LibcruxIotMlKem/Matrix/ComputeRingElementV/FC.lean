...
theorem compute_ring_element_v_fc
  ...
  · -- POST: (∃ spec_out, hacspec = .ok spec_out ∧ PolyMatches result3 spec_out)
    -- ∧ hacspec = .ok (lift_poly result3); `h_hac` (the A/C/B/compose/glue/D′ chain) exposed
    -- so the Rust-annotation discharge recovers `spec_out = lift_poly result3`.
    have h_hac : hacspec_ml_kem.matrix.compute_ring_element_v
        (lift_t_as_ntt_from_public_key public_key K) (lift_vec_slice r_as_ntt K)
        (lift_poly error_2) (lift_poly message) = .ok (lift_poly result3) := by
