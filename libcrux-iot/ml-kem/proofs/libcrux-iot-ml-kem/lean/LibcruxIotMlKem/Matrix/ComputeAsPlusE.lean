...
def multiply_matrix_by_column_at_loop.body
    ...
    RustM (ControlFlow ((CoreModels.core.ops.range.Range Std.Usize) ×
...
def multiply_matrix_by_column_at_loop
    ...
    RustM (Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize) := do
  loop
...
def multiply_matrix_by_column_at
    ...
    (m : Std.Array (Std.Array (Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize) RANK) RANK)
    (vector : Std.Array (Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize) RANK)
    (i : Std.Usize) : RustM (Std.Array hacspec_ml_kem.parameters.FieldElement 256#usize) := do
...
theorem compute_As_plus_e_loop0_step_lemma_fc
  ...
  · -- `Some k` branch.
    ...
    simpa [Aeneas.Std.RustM.holds, pure, Pure.pure, Std.Do.Triple, Std.Do.WP.wp,
           Std.Do.PredTrans.apply, Std.Do.PostCond.noThrow] using h_inv_pure
...
def rows_inv {K : Std.Usize}
    ...
    Std.Usize → TVec K → Acc → RustM Prop :=
...
theorem compute_As_plus_e_loop1_step_lemma_fc
  ...
  · -- `Some k` branch (i = k).
    ...
    obtain ⟨h_acc_final_lane, h_acc_final_bnd⟩ := by
      simpa [Stage2UseCacheFC.row_i_inv, Aeneas.Std.RustM.holds, Std.Do.Triple, Std.Do.WP.wp]
...
theorem compute_As_plus_e_row0_finalize_fc
  ...
  obtain ⟨h_row0_lane, _h_row0_bnd, _h_row0_cache_pop, _h_row0_cache_unch⟩ := by
    simpa [Aeneas.Std.RustM.holds, Std.Do.Triple, Std.Do.WP.wp] using h_row0_inv
...
theorem matrix_add_polynomials_eq_ok
  ...
  have hpure : ∀ k : Nat, k < (256#usize : Std.Usize).val →
      (hacspec_ml_kem.matrix.add_polynomials.closure.Insts.CoreOpsFunctionFnMutTupleUsizeFieldElement).call_mut
    ...
    change (do
      ...
      let fe2 ← hacspec_ml_kem.parameters.FieldElement.new i5
      RustM.ok (fe2, lhs, rhs)) = RustM.ok (f k, lhs, rhs)
    rw [h_lhs_idx]; simp only [bind_tc_ok]
    rw [h_rhs_idx]; simp only [bind_tc_ok]
    -- The closure inlines `FieldElement.add`'s body ending in `new i5`; fold it via
    -- `congrArg (· >>= wrap)` (bind-assoc is not defeq on the stuck do-block).
...
theorem multiply_matrix_by_column_at_eq
  ...
  | .ok r, h =>
    have hr : r = col_loop_result_at_step matrix_A s_as_ntt i.val K.val := by
      simpa [Aeneas.Std.RustM.holds, pure, Pure.pure, Std.Do.Triple, Std.Do.WP.wp, Std.Do.PredTrans.apply, Std.Do.PostCond.noThrow] using h
    rw [hr]
...
theorem multiply_matrix_by_column_eq
  ...
  have hpure : ∀ k : Nat, k < K.val →
      (hacspec_ml_kem.matrix.multiply_matrix_by_column.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256
        K).call_mut
    ...
    -- The closure inlines the per-column loop (= the local `multiply_matrix_by_column_at`,
    -- which reuses the identical loop body); connect via bind-assoc, then fold `h_mmbc_at`.
    have hconn :
        (hacspec_ml_kem.matrix.multiply_matrix_by_column.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256 K).call_mut
      ...
      simp [hacspec_ml_kem.matrix.multiply_matrix_by_column.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256,
            ...
            hacspec_ml_kem.matrix.multiply_matrix_by_column_at,
            hacspec_ml_kem.matrix.multiply_matrix_by_column_at_loop,
            hacspec_ml_kem.matrix.multiply_matrix_by_column_at_loop.body, bind_assoc]
      try rfl
      try (intro v _; rfl)
    rw [hconn, h_mmbc_at]; simp only [bind_tc_ok]
    rw [hk_val]
  ...
  have h_from_fn := libcrux_iot_ml_kem.Util.CreateI.from_fn_pure_eq
    ...
    (N := K)
    (inst := (hacspec_ml_kem.matrix.multiply_matrix_by_column.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256 K))
...
theorem hacspec_compute_As_plus_e_eq_of_lane_eq
  ...
  have hpure : ∀ r : Nat, r < K.val →
      (hacspec_ml_kem.matrix.add_vectors.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256 K).call_mut
    ...
    show (do
             ...
             let a4 ← hacspec_ml_kem.matrix.add_polynomials a2 a3
             .ok (a4, P_arr, lift_vec error_as_ntt)) = _
    ...
    -- Beta-reduce f_out r.
    show RustM.ok (⟨List.map _ (List.range 256), _⟩, P_arr, lift_vec error_as_ntt) =
         RustM.ok ((lift_vec t_as_ntt_final).val[r]!, P_arr, lift_vec error_as_ntt)
  ...
  have h_from_fn := libcrux_iot_ml_kem.Util.CreateI.from_fn_pure_eq
    ...
    (N := K)
    (inst := (hacspec_ml_kem.matrix.add_vectors.closure.Insts.CoreOpsFunctionFnMutTupleUsizeArrayFieldElement256 K))
...
theorem compute_As_plus_e_fc
    ...
    ⦃ ⇓ p => ⌜ (∃ spec_out,
                ...
                ∧ hacspec_ml_kem.matrix.compute_As_plus_e
                    (lift_matrix_from_slice matrix_A K)
                    (lift_vec s_as_ntt) (lift_vec error_as_ntt) = .ok (lift_vec p.1) ⌝ ⦄ := by
  -- Length facts.
  ...
  obtain ⟨h_rows_done, h_rows_undone, h_rows_bnd⟩ := by
    simpa [Stage3MontStripFC.rows_inv, Aeneas.Std.RustM.holds, Std.Do.Triple, Std.Do.WP.wp,
  ...
  · unfold libcrux_iot_ml_kem.matrix.compute_As_plus_e
    ...
    show (do
        ...
        Aeneas.Std.RustM.ok (t2', cache1, accumulator2'))
      = Aeneas.Std.RustM.ok (t_as_ntt2, cache1, accumulator2)
    ...
    | ok v0 =>
      ...
      change (do
          ...
          let pre4' ← libcrux_iot_ml_kem.polynomial.PolynomialRingElement.add_standard_error_reduce
              portable_ops_inst pre2' pre3'
          Aeneas.Std.RustM.ok (index_mut_back1 pre4')) = Aeneas.Std.RustM.ok a at h_fin_eq
      ...
      | ok t1 =>
        ...
        | ok v2 =>
          ...
          change (do
              ...
              let pre4' ← libcrux_iot_ml_kem.polynomial.PolynomialRingElement.add_standard_error_reduce
                  portable_ops_inst pre2 pre3'
              Aeneas.Std.RustM.ok (imb1 pre4')) = Aeneas.Std.RustM.ok a at h_fin_eq
          ...
          | ok pre3 =>
            ...
            | ok pre4 =>
              ...
              have h_a_eq : imb1 pre4 = a := Aeneas.Std.RustM.ok.inj h_fin_eq
  ...
  · -- POST (canonical-output form). p.1 = t_as_ntt2. Witness spec_out := lift_vec t_as_ntt2
    ...
    refine ⟨⟨lift_vec t_as_ntt2, h_hacspec, ?_⟩, h_hacspec⟩
