...
opaque arithmetic.power2round_vector
  ...
  Slice (polynomial.PolynomialRingElement SIMDUnit) → Slice
    (polynomial.PolynomialRingElement SIMDUnit) → RustM ((Slice
...
opaque simd.portable.encoding.commitment.serialize
  :
  simd.portable.vector_type.Coefficients → Slice Std.U8 → RustM (Slice
    Std.U8)
...
opaque simd.portable.encoding.t0.deserialize
  :
  Slice Std.U8 → simd.portable.vector_type.Coefficients → RustM
...
opaque simd.portable.sample.rejection_sample_less_than_field_modulus
  : Slice Std.U8 → Slice Std.I32 → RustM (Std.Usize × (Slice Std.I32))
...
def I64.Insts.Libcrux_secretsIntCastOps.as_u64 (x : Std.I64) : RustM Std.U64 := do
...
def I64.Insts.Libcrux_secretsIntCastOps.as_i32 (x : Std.I64) : RustM Std.I32 := do
  let i ← traits.Declassify.Blanket.declassify x
  let i1 ← lift (Std.IScalar.cast .I32 i)
  traits.Classify.Blanket.classify i1

/-- `U64 → I32` cast (montgomery reduction's `k` extraction). -/
def U64.Insts.Libcrux_secretsIntCastOps.as_i32 (x : Std.U64) : RustM Std.I32 := do
  let i ← traits.Declassify.Blanket.declassify x
  let i1 ← lift (Std.UScalar.hcast .I32 i)
  traits.Classify.Blanket.classify i1

/-- `I32 → I64` cast (montgomery multiply widening). -/
def I32.Insts.Libcrux_secretsIntCastOps.as_i64 (x : Std.I32) : RustM Std.I64 := do
  let i ← traits.Declassify.Blanket.declassify x
