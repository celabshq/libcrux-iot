...
theorem core_models_Slice_Insts_index_mut_RangeUsize_spec
  ...
  obtain ⟨ns, hns_eq, hns_val⟩ := Slice.subslice_le_eq s ⟨r.start, r.end⟩ h0 h1
  -- CoreModels v0.3.12 supplies this instance itself: `index_mut` delegates to
  ...
  -- `fun ss => ⟨s.val.setSlice! start ss.val, _⟩`, so the `Slice.update_subslice`
  -- detour the previous hand-written model needed (and with it `HaxToRange`) is
  -- gone: the third conjunct is now `rfl`.
