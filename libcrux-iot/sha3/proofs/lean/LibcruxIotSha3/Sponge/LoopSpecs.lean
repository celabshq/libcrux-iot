...
Until merge 9b285d1b this was two passes -- one filling a 25-lane
`state_flat` scratch array, one XOR-ing it in -- and the proofs came in
matching halves (`load_block_2u32_loop0_spec`, `..._loop1_spec`). Those are
replaced here by a single spec: `state_flat` no longer exists, so there is no
intermediate to state a spec about.

The composed statement is unchanged, because `j ↦ 5*(j%5) + j/5` is injective
on `[0, 25)`: each iteration writes a distinct lane and reads only that lane's
incoming value, so fusing the passes cannot change the result.

Preconditions (the union of the two old specs'):
...
/-- The lane value at impl-flat-index `5*(j%5) + j/5` after the fused loop's
    ...
    This is the old `loop1_lane_at` with `state_flat.val[j]!` replaced by the
    interleave that the fused body now computes in place. -/
def load_lane_at
...
theorem state.load_block_2u32_loop_spec
  ...
  · intro acc k h_ge h_le_k hinv
    ...
    · rintro ⟨hge, hiter1_eq⟩
      ...
      refine triple_of_ok_local rfl (pure_prop_holds ⟨h_acc_i, ?_, ?_⟩)
    ...
    · rintro ⟨hi_eq, hk_lt, hiter1_end, hiter1_start⟩
      ...
      have hk_25 : k.val < 25 := by
        ...
        omega
      -- byte-window bounds (from the old loop0 body)
      ...
      -- distinct lane, which is what lets the two old passes fuse.
      have h_inj_dec :
          ∀ (a b : Fin 25), a ≠ b →
      ...
      · intro j hj_start hj_25
        ...
        · -- j = k: the lane just written.
          ...
          -- (Lifted from the old loop0 body proof; the names line up.)
